...
RSpec.describe "Student Profiles & Direct Messaging Routing Matrix", type: :request do
  ...
  # =========================================================================
  # 1. PROFILE METRICS DATA AGGREGATION PIPELINE TEST
  # =========================================================================
  describe "GET /users/:id/profile (Upgraded Metric Dashboard State)" do
    it "successfully compiles multi-table SQL joins and group arrays for the views" do
      # Seed explicit practice logs to verify the controller's analytical collection groups
      ...
      # Fire the profile request trace
      get "/u/#{student_target.id}"

      expect(response).to have_http_status(:ok)

      # Verify instance variables load and compile accurately
  ...
  # =========================================================================
  # 2. DETERMINISTIC PRIVATE ROOM FACTORY SYSTEM TEST
  # =========================================================================
  describe "GET /users/:id/chat (Direct Message Routing Portal)" do
    it "calculates unique sorted room handles and builds a private direct room if missing" do
      ...
      # Trigger the direct chat route to connect admin_teacher and student_target
      get "/u/#{student_target.id}/chat"

      expect(response).to have_http_status(:ok)
      expect(Room.where(is_private: true).count).to eq(1)

      # Verify that the generated room name contains our deterministic private tracking signatures
